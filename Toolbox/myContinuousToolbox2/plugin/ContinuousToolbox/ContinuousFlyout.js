...
class ContinuousFlyout extends Blockly.VerticalFlyout {
  ...
  recordScrollPositions() {  
    this.scrollPositions = [];
    const categoryLabels = this.contents.filter((button) => {
		if (!button.type=="label") return false;
		return button;
	});
    for (const button of categoryLabels) {
      if (button.type=="label") {
        this.scrollPositions.push({
		name: Blockly.Msg[button.element.info.text.replace("%{BKY_","").replace("}","")],
          position: button.element.position,
        });
      }
    }
  }
