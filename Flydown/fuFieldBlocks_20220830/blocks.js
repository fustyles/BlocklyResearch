...
/*
	Blockly.Blocks["test"] = {
		init:  function() {
			...
			this.appendDummyInput()
				.appendField(new myFieldFlydown.eventparam('\u2615', '#fff', workspace, blocksXML1, 'field1'));	
				
			this.appendDummyInput()
				.appendField("Flydown2");	
				
			var blocksXML2 = ['<block type="controls_if"></block>','<block type="text"><field name="TEXT"></field></block>'];
			this.appendDummyInput()
				.appendField(new myFieldFlydown.eventparam('\u2614', '#fff', workspace, blocksXML2, 'field2'));	
...
document.addEventListener('DOMContentLoaded', function() {
	...
	Blockly.fuFieldBlocks = function(workspaceOptions) {
	  Blockly.fuFieldBlocks.superClass_.constructor.call(this, workspaceOptions);
	};
	Blockly.utils.object.inherits(Blockly.fuFieldBlocks, Blockly.VerticalFlyout);
	 ...
	 * @const
	 */
	Blockly.fuFieldBlocks.prototype.previousCSSClassName_ = '';
	 ...
	 * @const
	 */
	Blockly.fuFieldBlocks.prototype.VERTICAL_SEPARATION_FACTOR = 1;
	 ...
	 */
	Blockly.fuFieldBlocks.prototype.createDom = function(cssClassName, backgroundCOLOR) {
	  /*
	  <g>
	 ...
	 */
	Blockly.fuFieldBlocks.prototype.setCSSClass = function(newCSSClassName) {
	 ...
	 *     blocks.
	 */
	Blockly.fuFieldBlocks.prototype.init = function(workspace) {
	 ...
	 * @private
	 */
	Blockly.fuFieldBlocks.prototype.position = function() {
	  return;
	}
	 ...
	 */
	Blockly.fuFieldBlocks.prototype.showAt = function(xmlList,x,y) {
	 ...
	 */
	Blockly.fuFieldBlocks.prototype.reflow = function() {
	...
	Blockly.fuFieldBlocks.prototype.onMouseMove_ = function(e) {
	  // override Blockly's flyout behavior for moving the flyout.
	  return;
	};

	Blockly.fuFieldBlocks.prototype.getX = function () {
	  return 0;
	};
	 ...
	 * @private
	 */
	Blockly.fuFieldBlocks.prototype.placeNewBlock_ = function(originBlock) {
	...
	Blockly.fuFieldBlocks.prototype.shouldHide = true;

	Blockly.fuFieldBlocks.prototype.hide = function() {
	  if (this.shouldHide) {
		Blockly.VerticalFlyout.prototype.hide.call(this);
		myFieldBlocks.eventparam.openFieldBlocks_ = null;
	  }
	...
	myFieldBlocks.eventparam = function (opt_value, opt_color, opt_workspace, opt_blocksXML, opt_fieldID, opt_validator) {
		...
		this.displayLocation = myFieldBlocks.eventparam.DISPLAY_BELOW;
		this.opt_workspace = opt_workspace;
		if (opt_blocksXML)
			this.opt_blocksXML_ = '<xml>'+opt_blocksXML.join("")+'</xml>';
		
		myFieldBlocks.eventparam.superClass_.constructor.call(
			this, opt_value, opt_validator);
	};
	Blockly.utils.object.inherits(myFieldBlocks.eventparam, Blockly.Field);

	myFieldBlocks.eventparam.prototype.EDITABLE = false;
	myFieldBlocks.eventparam.timeout = 500;
	myFieldBlocks.eventparam.isInDrag = false;
	myFieldBlocks.eventparam.openFieldBlocks_ = null;
	myFieldBlocks.eventparam.showPid_ = 0;
	myFieldBlocks.eventparam.DISPLAY_BELOW = "BELOW";
	myFieldBlocks.eventparam.DISPLAY_RIGHT = "RIGHT";
	myFieldBlocks.eventparam.DISPLAY_LOCATION = myFieldBlocks.eventparam.DISPLAY_BELOW;
	myFieldBlocks.eventparam.prototype.fieldCSSClassName = 'blocklyFieldBlocksField';
	myFieldBlocks.eventparam.prototype.flyoutCSSClassName = 'blocklyFieldBlocksFlydown';

	myFieldBlocks.eventparam.prototype.init = function (block) {
		myFieldBlocks.eventparam.superClass_.init.call(this, block);
		...
		this.mouseUpWrapper_ =
			Blockly.bindEventWithChecks_(document, 'mouseup', this,
				function (_event) {
					this.isInDrag = false;
					myFieldBlocks.eventparam.hide();
				}
			);
	};

	myFieldBlocks.eventparam.prototype.onMouseOver_ = function (e) {
		if (!this.sourceBlock_.isInFlyout) { // [lyn, 10/22/13] No flydowns in a flyout!
			myFieldBlocks.eventparam.showPid_ =
				window.setTimeout(this.showFlydownMaker_(), myFieldBlocks.eventparam.timeout);
		}
	...
	myFieldBlocks.eventparam.prototype.onMouseOut_ = function (e) {
		// Clear any pending timer event to show flydown
		window.clearTimeout(myFieldBlocks.eventparam.showPid_);
		e.stopPropagation();
	};

	myFieldBlocks.eventparam.prototype.showFlydownMaker_ = function () {
		var field = this; // Name receiver in variable so can close over this variable in returned thunk

		return function () {
			if (myFieldBlocks.eventparam.showPid_ !== 0 &&
				!this.isInDrag &&
	...
	myFieldBlocks.eventparam.prototype.showFlydown_ = function () {
		Blockly.hideChaff();
		...
		if (this.displayLocation == myFieldBlocks.eventparam.DISPLAY_BELOW) {
		...
		myFieldBlocks.eventparam.openFieldBlocks_ = this;
	};

	myFieldBlocks.eventparam.hide = function () {
		// Clear any pending timer event to show flydown.
		window.clearTimeout(myFieldBlocks.eventparam.showPid_);
	...
	myFieldBlocks.eventparam.prototype.onHtmlInputChange_ = function (e) {
	...
	myFieldBlocks.eventparam.prototype.dispose = function () {
		  if (myFieldBlocks.eventparam.openFieldBlocks_ == this) {
			myFieldBlocks.eventparam.hide();
		}
	...
	myFieldBlocks.eventparam.prototype.initView = function () {
	...
	var fuFieldBlocksTimer = setInterval(function() {

		var workspace = Blockly.getMainWorkspace();
		if (!workspace) 
			return;
		else
			clearInterval(fuFieldBlocksTimer)
		
		var flydown = new  Blockly.fuFieldBlocks(new Blockly.Options({scrollbars:  true }));
		...
		Blockly.Blocks["test"] = {
			init:  function() {
				...
				var blocksXML1 = ['<block type="math_number"><field name="NUM">0</field></block>','<block type="variables_get"><field name="VAR">i</field></block>'];
				this.appendDummyInput()
					.appendField(new myFieldBlocks.eventparam('\u2615', '#fff', workspace, blocksXML1, 'field1'));	
				...
				var blocksXML2 = ['<block type="controls_if"></block>','<block type="text"><field name="TEXT"></field></block>'];
				this.appendDummyInput()
					.appendField(new myFieldBlocks.eventparam('\u2614', '#fff', workspace, blocksXML2, 'field2'));	
