...
BlockFactory.updatePreview = function() {
  ...
  var warnExistingBlock = function(blockType) {
	...
	allA.forEach(( link, index ) => {
		if (link.id == "dropdown_" + blockType) {
			...
			if (!link.selected) {
				setTimeout(function () {
					var saveButton = document.getElementById('saveToBlockLibraryButton');
  ...
  try {
	...
	if (warnExistingBlock(blockType)!=null) {
